...
namespace Psychology
{
    public class JobGiver_Compulsion : ThinkNode_JobGiver
    {
        ...
        protected override Job TryGiveJob(Pawn pawn)
        {
            ...
            if (!pawn.WorkTagIsDisabled(WorkTags.Cleaning) && pawn.Map.listerFilthInHomeArea.FilthInHomeArea.Count > 0)
            {
                ...
                if (closestFilth != null && pawn.CanReserveAndReach(closestFilth, PathEndMode.Touch, Danger.Some))
                {
                    ...
                    for (int i = 0; i < 100; i++)
                    {
                        ...
                        if (intVec.InBounds(map) && intVec.GetRoom(map) == room) // TEST IN 1.3 old - GetRoom(map, RegionType.Set_Passable
                        {
            ...
            if (!pawn.WorkTagIsDisabled(WorkTags.Hauling) && pawn.Map.listerHaulables.ThingsPotentiallyNeedingHauling().Count > 0)
            {
