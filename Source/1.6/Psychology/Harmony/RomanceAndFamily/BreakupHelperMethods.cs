...
public static class RomanceHelperMethods
{
    ...
    {
        //TryGainOpinion(pawn, otherPawn, ThoughtDefOf.CheatedOnMe);
        // pawn.needs.mood.thoughts.memories.TryGainMemory(ThoughtDefOf.CheatedOnMe, otherPawn); // BUGGED
        //pawn.needs.mood.thoughts.memories.TryGainMemory(ThoughtDefOfPsychology.CheatedOnMeCodependent, otherPawn); // BUGGED
        //TryGainMood(pawn, otherPawn, CheatedOnMeMoodDef); // BUGGED
        //ToDo: make ICheatedOnThemMood ?
        //TryGainMood(pawn, otherPawn, ThoughtDefOfPsychology.ICheatedOnThemMood);
    }
    ...
    public static int GetStageIndex(float offset, List<Pair<int, float>> baseEffectList) // What does this even do??
    {
        ...
        {
            //Log.Error("allowedStageIndices is null or empty."); // Commented out beacuse its spamming error sometimes. Doesnt seem to break anything tho
            return -1;
        }
