...
public static class PawnDiedOrDownedThoughtUtility_AppendThoughtsPatch
{
    ...
    public static void AppendPsychologyThoughts(Pawn victim, DamageInfo? dinfo, PawnDiedOrDownedThoughtsKind thoughtsKind, ref List<IndividualThoughtToAdd> outIndividualThoughts, ref List<ThoughtDef> outAllColonistsThoughts)
    {
        ...
        //List<Pawn> allPawnsSpawned = victim.Map.mapPawns.AllPawnsSpawned; // Causes NPE
        if (flag3)
        {
        ...
        if (thoughtsKind == PawnDiedOrDownedThoughtsKind.Banished && victim.IsColonist)
        {
            foreach (Pawn pawn2 in PawnsFinder.AllMapsCaravansAndTravellingTransporters_Alive) {
                if (pawn2.Faction == Faction.OfPlayer && pawn2.needs.mood != null)
                {
                    ...
                    {
                        outIndividualThoughts.Add(new IndividualThoughtToAdd(ThoughtDefOfPsychology.ColonistAbandonedBleedingHeart, pawn2, null, 1f, 1f));
                    }
